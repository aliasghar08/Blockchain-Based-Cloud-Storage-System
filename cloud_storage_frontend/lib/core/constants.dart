class Constants {
  static const String pinataJwt = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..FirfDasrHJsuwzpt4rIgp8Ik8r-STBeGXv6vsnEnOX4';
  
  // iOS Simulator: use 127.0.0.1. Physical iPhone on LAN: use your Mac's local IP (e.g. 192.168.x.x).
  static const String rpcUrl = 'http://127.0.0.1:8545';
  static const int chainId = 31337;
  // TODO: Replace with your deployed contract address after running `npx hardhat run scripts/deploy.js`
  static const String contractAddress = 'YOUR_CONTRACT_ADDRESS_HERE';

  // Secure Storage Keys
  static const String privateKeyStorageKey = 'wallet_private_key';
}
